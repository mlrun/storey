...
@pytest.mark.parametrize('flush_interval', [0, 1000])
def test_aggregate_and_query_with_different_windows(setup_teardown_test, partitioned_by_key, flush_interval):
    table = Table(setup_teardown_test, V3ioDriver(), partitioned_by_key=partitioned_by_key, flush_interval_milli=flush_interval)
...
def test_write_cache_with_aggregations(setup_teardown_test, flush_interval):
    table = Table(setup_teardown_test, V3ioDriver(), flush_interval_milli=flush_interval)
    ...
    other_table = Table(setup_teardown_test, V3ioDriver(), flush_interval_milli=flush_interval)
