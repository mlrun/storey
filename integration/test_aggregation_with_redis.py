...
from .integration_test_utils import (append_return, setup_redis_teardown_test,
                                     test_base_time)
...
def test_aggregate_with_fixed_windows_and_query_past_and_future_times(setup_redis_teardown_test, partitioned_by_key, flush_interval,
                                                                      fixed_window_type,redis_driver):
...
@pytest.mark.parametrize('partitioned_by_key', [True, False])
@pytest.mark.parametrize('flush_interval', [None, 1])
def test_aggregate_and_query_with_different_sliding_windows(setup_redis_teardown_test, partitioned_by_key, flush_interval, redis_driver):
    table = Table(setup_redis_teardown_test, redis_driver, partitioned_by_key=partitioned_by_key, flush_interval_secs=flush_interval)
    ...
    tables = [table, Table(setup_redis_teardown_test, redis_driver)]  # test on previous table and on new table
    expected_results = [
...
@pytest.mark.parametrize('partitioned_by_key', [True, False])
@pytest.mark.parametrize('flush_interval', [None, 1])
def test_aggregate_and_query_with_different_fixed_windows(setup_redis_teardown_test, partitioned_by_key, flush_interval,redis_driver):
    table = Table(setup_redis_teardown_test, redis_driver, partitioned_by_key=partitioned_by_key, flush_interval_secs=flush_interval)
    ...
    table_name = setup_redis_teardown_test
    tables = [table_name, table, Table(setup_redis_teardown_test, redis_driver)]  # test on previous table and on new table
    expected_results = [
        {'col1': 10, 'number_of_stuff_sum_1h': 17.0, 'number_of_stuff_min_1h': 8.0,
         'number_of_stuff_max_1h': 9.0, 'number_of_stuff_avg_1h': 8.5},
    ...
    for table in tables:
        if isinstance(table, str):
            tmp_table = Table(table_name, redis_driver)
...
def test_query_virtual_aggregations_flow(setup_redis_teardown_test, use_parallel_operations, redis_driver):
...
def tst_write_read_first_last(setup_redis_teardown_test, redis_driver):
    table = Table(setup_redis_teardown_test, redis_driver, flush_interval_secs=None)
    controller = build_flow([
        SyncEmitSource(),
...
def test_non_existing_key_query_by_key_from_v3io_key_is_list(setup_redis_teardown_test, redis_driver):
    table = Table(setup_redis_teardown_test, redis_driver)
...
def test_multiple_keys_int(setup_redis_teardown_test,redis_driver):
    t0 = pd.Timestamp(test_base_time)
    data = pd.DataFrame(
        {
...
def test_column_begin_t(setup_redis_teardown_test, redis_driver):
    t0 = pd.Timestamp(test_base_time)
    data = pd.DataFrame(
        {
            'key_column': ['a', 'b'],
...
def test_aggregate_float_key(setup_redis_teardown_test, redis_driver):
    t0 = pd.Timestamp(test_base_time)
    data = pd.DataFrame(
        {
            'key_column2': [5.6, 8.6],
...
def test_aggregate_and_query_persist_before_advancing_window(setup_redis_teardown_test, flush_interval, redis_driver):
    table = Table(setup_redis_teardown_test, redis_driver, flush_interval_secs=flush_interval)
    controller = build_flow([
        SyncEmitSource(),
...
def test_aggregate_and_query_by_key_with_holes(setup_redis_teardown_test, redis_driver):
    table = Table(setup_redis_teardown_test, redis_driver, partitioned_by_key=False, flush_interval_secs=None)
