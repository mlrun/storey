...
class AggregationBuckets:
    def __init__(self, name, aggregation, base_time, max_value, is_hidden, initial_windows, initial_data=None):
        ...
        self.period_millis = initial_windows.period_millis
        self.max_window_millis = initial_windows.max_window_millis
        self.total_number_of_buckets = initial_windows.total_number_of_buckets
    ...
    def add_to_window(self, is_hidden, new):
        ...
        if current:
            ...
            current = new
            if is_hidden:
                self.hidden_windows = new
            else:
                self.explicit_windows = new
        if self.total_number_of_buckets < current.total_number_of_buckets:
            self.total_number_of_buckets = current.total_number_of_buckets
        if self.max_window_millis < current.max_window_millis:
            self.max_window_millis = current.max_window_millis
    ...
    def initialize_from_data(self):
        ...
        if self.explicit_windows and self.explicit_windows.max_window_millis == self.max_window_millis:
            self.last_bucket_start_time = self.explicit_windows.get_window_start_time_by_time(self.base_time)
        else:
            self.last_bucket_start_time = self.hidden_windows.get_window_start_time_by_time(self.base_time)
        self.first_bucket_start_time = \
            self.last_bucket_start_time - (self.total_number_of_buckets - 1) * self.period_millis

        start_index = int((self.base_time - last_time) / self.period_millis)
        ...
        if start_index < len(aggregation_bucket_initial_data[last_time]):
            ...
            if first_time and bucket_index >= 0 and self.base_time > first_time:
