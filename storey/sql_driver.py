...
class SQLDriver(Driver):
    ...
    def __init__(
        ...
        self._primary_key = (
            ...
            else self._extract_list_of_keys(primary_key)
        )
    ...
    async def _save_key(
        ...
        try:
            df.to_sql(
                table.name, con=self._sql_connection, if_exists="append", index=False
            )
    ...
    async def _get_all_fields(self, key, table):
        ...
        results = pd.read_sql(
            query, con=self._sql_connection, parse_dates=self._time_fields
    ...
    def _update_by_key(self, key, data, table):
        ...
        update_clause = " ,".join(
            [
                f'{table.name}.{key}="{value}"'
                for key, value in data.items()
                if key not in self._primary_key
            ]
        )
        sql_statement = (
            f"UPDATE {table} as {table.name} SET {update_clause} where {where_clause}"
        )
